...
public class PlayerEvents implements Listener {
    ...
    public void on(PlayerDeathEvent event) {
        Bukkit.getScheduler().runTaskLater(instance, () -> event.getEntity().spigot().respawn(), 3L);
    }
    ...
    public void on(PlayerJoinEvent event) {
        ...
        playerBank.load(player);

        cc.getCombatTime().put(player, 0L);
        cc.getLastHitters().put(player, null);

        Bukkit.getScheduler().runTaskLater(instance, () -> player.teleport(wc.getSpawnPoint()), 10L);
